...
-- Bir urun birden fazla kategoride yer alabilir. Kategoriye ozel sira bu
-- bag tablosunda tutulur; urunler.kategori_id geriye uyumlu ana kategoridir.
...
create index if not exists urun_kategorileri_kategori_sira_idx
  on urun_kategorileri(kategori_id, sira, urun_id);
create index if not exists urun_kategorileri_urun_idx
  on urun_kategorileri(urun_id, kategori_id);

insert into urun_kategorileri (kategori_id, urun_id, sira)
select kategori_id, id, sira
from urunler
...
alter table urun_kategorileri enable row level security;
...
drop policy if exists "menu_herkes_okur" on urun_kategorileri;
...
create policy "menu_herkes_okur" on urun_kategorileri for select using (true);
...
drop policy if exists "menu_admin_yazar" on urun_kategorileri;
...
create policy "menu_admin_yazar" on urunler for all to authenticated
  using (is_admin()) with check (is_admin());
create policy "menu_admin_yazar" on urun_kategorileri for all to authenticated
  using (is_admin()) with check (is_admin());
...
-- ---------- Coklu kategori ve kategoriye ozel urun sirasi ----------
-- Bu son tanimlar yukaridaki geriye uyumlu RPC'leri yeni bag tablosuna tasir.
